...
FFMPEG_PATH = '"ffmpeg"'
SOUNDS_PATH = os.path.join(Path().resolve().parent, 'sounds', 'original')
...
print('If a WAV-file has "-vocals" anywhere on its path, it will also apply a bandpass filter to isolate vocals.')
print('')
...
for wav in (Path(SOUNDS_PATH).rglob("*.[wW][aA][vV]")):
    ...
    subprocess.run('ffmpeg -i "{0}" -af "adelay=10000|10000" -y "{1}"'.format(wav, pad_wav), shell=True)
    
    subprocess.run('ffmpeg-normalize "{0}" -t -15 -o "{1}"'.format(pad_wav, norm_wav), shell=True)
    os.remove(pad_wav)
    
    subprocess.run('ffmpeg -i "{0}" -af silenceremove=1:0:-50dB -y "{1}"'.format(norm_wav, trim_wav), shell=True)
    os.remove(norm_wav)
    
    subprocess.run('ffmpeg -i "{0}" -ar 44100 -y "{1}"'.format(trim_wav, rate_wav), shell=True)
    os.remove(trim_wav)
    
    if '-vocal' in str(wav):
        subprocess.run('ffmpeg -i "{0}" -af lowpass=4000,highpass=250 -y "{1}"'.format(rate_wav, new_wav), shell=True)
        os.remove(rate_wav)        
